# 🛡️ Google Cybersecurity Certificate

**Issued by:** Google via Coursera  
**Status:** ✅ Completed  


---

## 📌 About This Repository

This repository documents my progress through the **Google Cybersecurity Certificate** — a 9-course professional certificate program covering foundational and applied cybersecurity skills. It includes my verified certification (upon completion), hands-on lab write-ups, and projects produced throughout the program.

All lab work was performed in controlled environments provided by the Google Cybersecurity Certificate program on Coursera. No real-world systems were accessed without authorization.

---

## 🏅 Certification

| Detail | Info |
|--------|------|
| **Certificate** | Google Cybersecurity Certificate |
| **Issuer** | Google / Coursera |
| **Status** | ✅ Completed |
| **Verified Badge** | *Will be linked upon completion via Credly* |
| **Certificate PDF** | [📄 PDF](./certificates/Google-Cybersecurity.pdf) |

---

## 📚 Course Progress

| # | Course | Status | Certificate |
|---|--------|--------|---------------|
| 1 | Foundations of Cybersecurity | ✅ Completed | [📄 PDF](./certificates/Foundations-of-Cybersecurity.pdf) |
| 2 | Play It Safe: Manage Security Risks | ✅ Completed | [📄 PDF](./certificates/Play-It-Safe-Manage-Security-Risks.pdf) |
| 3 | Connect and Protect: Networks and Network Security | ✅ Completed | [📄 PDF](./certificates/Connect-and-Protect-Networks-and-Network-Security.pdf) |
| 4 | Tools of the Trade: Linux and SQL | ✅ Completed | [📄 PDF](./certificates/Tools-of-the-Trade-Linux-and-SQL.pdf) | 
| 5 | Assets, Threats, and Vulnerabilities | ✅ Completed | [📄 PDF](./certificates/Assets-Threats-and-Vulnerabilities.pdf) | 
| 6 | Sound the Alarm: Detection and Response | ✅ Completed | [📄 PDF](./certificates/Sound-the-Alarm-Detection-and-Response.pdf) | 
| 7 | Automate Cybersecurity Tasks with Python | ✅ Completed | [📄 PDF](./certificates/Automate-Cybersecurity-Tasks-with-Python.pdf) | 
| 8 | Put It to Work: Prepare for Cybersecurity Jobs | ✅ Completed | [📄 PDF](./certificates/Put-it-to-Work-Prepare-for-Cybersecurity-Jobs.pdf) |
| 9 | Accelerate Your Job Search with AI | ✅ Completed | [📄 PDF](./certificates/Accelerate-Your-Job-Search-with-AI.pdf) |


---

## 🔬 Labs & Projects

| Lab / Project | Course | Topic | Status | Folder |
|---------------|--------|-------|--------|--------|
| Security Audit — Botium Toys | Course 2 | Risk assessment, controls & compliance | ✅ Completed | [📁 labs/security-audit](./labs/security-audit/) |
| Network Traffic Analysis | Course 3 | Wireshark, packet capture | ✅ Completed | [📁 labs/network-traffic-analysis](./labs/network-traffic-analysis/) |
| Linux File Permissions | Course 4 | chmod, user management | ✅ Completed | [📁 labs/linux-permissions](./labs/linux-permissions/) |
| SQL Security Filters | Course 4 | Querying & filtering logs | ✅ Completed | [📁 labs/sql-filters](./labs/sql-filters/) |
| Vulnerability Assessment | Course 5 | NIST framework, risk scoring | ✅ Completed | [📁 labs/vulnerability-assessment](./labs/vulnerability-assessment/) |
| Incident Handler's Journal | Course 6 | Detection & response documentation | ✅ Completed | [📁 labs/incident-handlers-journal](./labs/incident-handlers-journal/) |
| Python File Parser | Course 7 | Automating log analysis | ✅ Completed | [📁 labs/python-file-parser](./labs/python-file-parser/) |


---

## Connect With Me

**LinkedIn:** [linkedin.com/in/joseph-guidici](https://www.linkedin.com/in/joseph-guidici/)
